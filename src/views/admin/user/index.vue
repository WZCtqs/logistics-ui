<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-input @keyup.enter.native="handleFilter" style="width: 200px;" class="filter-item" placeholder="账号"
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit
      ...
      <el-table-column align="center" label="操作" width="280">
        <template slot-scope="scope">
          ...
          <el-button v-if="sys_user_reset" size="small" type="warning" @click="resetPWD(scope.row)">重置密码
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :model="form" :rules="rules" ref="form" label-width="100px">
        <el-form-item label="账号" prop="username">
          <el-input v-model="form.username" placeholder="请输账号" :disabled="dialogStatus === 'update'"></el-input>
        </el-form-item>
        <el-form-item label="用户名" prop="realname">
          <el-input v-model="form.realname" placeholder="请输用户名"></el-input>
        </el-form-item>
...
<script>
  import { addObj, delObj, fetchList, getObj, putObj, resetObj } from '@/api/user'
  ...
  export default {
    ...
    methods: {
      ...
      resetPWD(row) {
        this.$confirm(
          '此操作将重置该用户(账号:' + row.username + ')的密码, 是否继续?',
          '提示',
          {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
          ...
          resetObj(obj)
            .then((res) => {
              if (res.data.data) {
                this.$alert('<div class="resetTip">' + res.data.msg + '</div><div style="color: #a1a4ad;text-align: center;margin-top: 12px">tip:此密码只显示一次</div>', '新密码', {
                  confirmButtonText: '确定',
