<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey'
            ...
            <el-row>
              ...
              <el-col :span="7">
                <el-form-item label="运费:">
                  <span>{{ props.row.transportCost }}</span>
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
      ...
      <!--label="运费">-->
      <!--<template slot-scope="scope">-->
      <!--<span>{{ scope.row.transportCost }}</span>-->
      <!--</template>-->
      <!--</el-table-column>-->
      <!--<el-table-column align="center"-->
      <!--label="编号">-->
    ...
    <el-dialog :title="textMap[dialogStatus]"
      ...
      <el-form :model="form"
        ...
        <el-form-item label="运费"
                      prop="username">
          <el-input v-model="form.transportCost"
                    placeholder="运费"
    ...
    <el-dialog title="油卡充值小票"
      ...
      <div class="pay-print"
           ref="print">
        <p class="pay-print-time">报销单号:{{printForm.no}}</p>
        <el-row>
        ...
        <!-- <el-row>
          ...
          <el-col :span="16">{{printForm.no}}</el-col>
        </el-row> -->
        <el-row>
...
export default {
  ...
  methods: {
    ...
    handleFilter () {
      ...
      if (this.listQuery.attribute === '') {
        delete this.listQuery.attribute
      }
      this.getList()
    },
    ...
    handleDelete (row) {
      if (this.multipleSelection.length === 0) {
        ...
        if (arr.length > 0) {
          ...
          this.$confirm('是否要删除选中的正常油卡信息', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
    ...
    getoilCardObjList (id) {
      getoilCardObj(id).then(response => {
        ...
        this.form.driverId = response.data.data.driverId
